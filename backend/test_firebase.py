...
def test_firebase_admin():
    """
    Test script to verify that the Firebase Admin SDK is working correctly.
    """
    ...
    try:
        ...
        # List users (limited to 1 for testing)
        users = auth.list_users(max_results=1)
        print(f"Successfully connected to Firebase Auth. Found {len(users.users)} user(s).")
